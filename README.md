...
| `line-continuation-in-code` | warn | Token pass. A `\<newline>` line continuation appears outside a `#define` directive and outside a string literal. Real code uses `\<nl>` only inside macros and (rarely) inside long string literals; using it to split an expression or function-body statement across lines is an IOCCC visual-shaping technique. |
